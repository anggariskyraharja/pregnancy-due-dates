...
export class Tab2Page {
  ...
  calculate(){
    if (this.from == null || this.to == null || this.num1 == null){
      ...
      if (todate > fromdate){
        ...
        var modifier = 266 + ((28 - this.num1)*-1);
        ...
        var difference_in_days = Math.floor(timediff/(1000*3600*24)) + 15;
        ...
        var fromdate = new Date(this.from);
        var timediff = todate.getTime()-fromdate.getTime();
        var difference_in_dayss = Math.floor(timediff/(1000*3600*24));
        ...
        var fromdate = new Date(this.to);
        var timediff = todate.getTime()-fromdate.getTime();
        var difference_in_daysss = Math.floor(timediff/(1000*3600*24)+ 15);
        ...
        this.pregnancytest.setDate(this.pregnancytest.getDate()+23-14);

        var newdate = new Date(this.to);
        this.babyheart = newdate;
        this.babyheart.setDate(this.babyheart.getDate()+35-14);

        var newdate = new Date(this.to);
        this.morningsickness = newdate;
        this.morningsickness.setDate(this.morningsickness.getDate()+63-14);

        var newdate = new Date(this.to);
        this.appointment = newdate;
        this.appointment.setDate(this.appointment.getDate()+70-14);

        var newdate = new Date(this.to);
        this.second = newdate;
        this.second.setDate(this.second.getDate()+98-14);

        var newdate = new Date(this.to);
        this.organ = newdate;
        this.organ.setDate(this.organ.getDate()+140-14);

        var newdate = new Date(this.to);
        this.heart = newdate;
        this.heart.setDate(this.heart.getDate()+142-14);

        var newdate = new Date(this.to);
        this.ultrasound = newdate;
        this.ultrasound.setDate(this.ultrasound.getDate()+147-14);

        var newdate = new Date(this.to);
        this.third = newdate;
        this.third.setDate(this.third.getDate()+196-14);
      }
    }
  }
  ...
  async presentAlert2() {
    const alert = await this.alertController.create({
      header: 'Wrong Value',
      message: 'Conception date must be before today date!',
      buttons: ['Okay']
    });
    await alert.present();
  }
